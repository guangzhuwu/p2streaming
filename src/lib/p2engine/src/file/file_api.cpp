...
#	define OPEN ::open 
		...
		boost::system::error_code& ec)
	{
		...
		int handle = OPEN(native_path_string(path).c_str()
			, mode_array[(mode & _rw_mask) % _countof(mode_array)]
#ifdef O_NOATIME
...
#	endif
		...
		boost::system::error_code& ec)
	{
		...
		open_mode_t const& m = mode_array[(mode & _rw_mask) % _countof(mode_array)];
		...
		HANDLE handle = CreateFileW(native_path_string(path).c_str()
			, m.rw_mode
			, (mode & lock_file) ? 0 : share_array[(mode & _rw_mask) % _countof(mode_array)]
