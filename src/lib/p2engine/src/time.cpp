...
#   else
#		error set precise_local_tick_count
#	endif
#endif
...
const uint64_t system_time::s_rough_local_tick_count = rough_local_tick_count();
const uint64_t system_time::s_precise_local_tick_count = precise_local_tick_count();
const ptime system_time::s_local_time_stamp = boost::posix_time::microsec_clock::local_time();
const ptime system_time::s_universal_time_stamp = boost::posix_time::microsec_clock::universal_time();

void system_time::sleep_microseconds(tick_type us)throw()
{
	if (!us)
	{
		usleep(0);
