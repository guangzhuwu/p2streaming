...
#   endif
	...
	std::string convert_to_native(std::string const& s)
	{
		...
		if(utf8_wchar(s, ws)!=conversionOK)
			return s;
		size_t size = wcstombs(0, ws.c_str(), 0);
		if (size==size_t(-1)) return s;
		std::string ret;
		ret.resize(size);
	...
	std::string convert_from_native(std::string const& s)
	{
		...
		std::wstring ws;
		ws.resize(s.size());
		size_t size = mbstowcs(&ws[0], s.c_str(), s.size());
		if (size == size_t(-1)) return s;
		std::string ret;
...
namespace p2engine {
	...
	std::string convert_to_native(std::string const& s)
	{
		static spinlock iconv_mutex;
		// only one thread can use this handle at a time
		spinlock::scoped_lock l(iconv_mutex);
