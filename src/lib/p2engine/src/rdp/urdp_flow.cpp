...
namespace
{
	...
	const time32_type UNINITIALIZE_TIME = -1;
...
spinlock s_urdp_flow_map_lock;
std::set<urdp_flow*> s_urdp_flow_map;
#endif

urdp_flow::shared_layer_sptr urdp_flow::create_shared_layer_for_acceptor(
	io_service& ios,
	endpoint& local_edp,
	boost::shared_ptr<acceptor_type> acceptor,
	error_code& ec
	)
{
	shared_layer_impl_sptr obj = shared_layer_impl_type::create(ios, local_edp, ec);
	if (!ec)
	{
		obj->register_acceptor(acceptor, ec);
		local_edp = obj->local_endpoint(ec);
		return boost::static_pointer_cast<basic_shared_layer_adaptor>(obj);
	}
	return boost::shared_ptr<shared_layer_type>();
}

boost::shared_ptr<urdp_flow> urdp_flow::create_for_active_connect(
	io_service& ios,
	...
	error_code& ec)
{
	...
	obj->m_shared_layer = shared_layer_impl_type::create(ios, local_edp, ec);
	obj->m_shared_layer->register_flow(obj, ec);
	...
	error_code& ec)
{
	...
	obj->m_shared_layer = boost::static_pointer_cast<shared_layer_impl_type>(sharedLayer);
	obj->m_shared_layer->register_flow(obj, ec);
...
#ifdef DEBUG_FLOW_CNT
	urdp_flow* ptr = 0;
	spinlock::scoped_lock lk(s_urdp_flow_map_lock);
...
void urdp_flow::__init()
{
	...
	if (m_timer)
	{
		m_timer->unregister_time_handler();
		m_timer->cancel();
	}
	else
	{
	...
	m_timer->register_time_handler(boost::bind(&this_type::__on_clock, this VC9_BIND_BUG_PARAM_DUMMY));
	m_t_close_base = now + IDLE_TIMEOUT;//we will close this if not connected in 20 seconds.
	m_t_last_on_clock = now;

	m_establish_time = UNINITIALIZE_TIME;
	m_shutdown = SD_NONE;
	...
	static atomic<uint32_t> s_sqno_seed(random<uint32_t>(0, 0xffffffffU));
	s_sqno_seed += random<uint32_t>(0xffU, 0xffffU);
	m_snd_una = m_snd_nxt = s_sqno_seed;
	m_slen = 0;
...
void urdp_flow::on_received(const safe_buffer& buf,
	const endpoint& from)
{
...
bool urdp_flow::is_open()const
{
	return m_shared_layer&&m_shared_layer->is_open();
}
...
void urdp_flow::async_connect(const endpoint& remoteEnp,
	const std::string& domainName, const time_duration& time_out)
{
	BOOST_ASSERT(m_shared_layer);
	if (m_state > INIT)
	{
...
void  urdp_flow::__handle_resolve(error_code ec, resolver_iterator itr,
	const std::string& domainName, const time_duration& time_out, op_stamp_t mark)
{
...
void urdp_flow::keep_async_receiving()
{
	if (!b_keep_recving_)
	{
		...
		get_io_service().post(make_alloc_handler(
			boost::bind(&this_type::__async_receive, SHARED_OBJ_FROM_THIS, this->op_stamp())
			));
	}
}
...
safe_buffer urdp_flow::make_punch_packet(error_code& ec, const endpoint& externalEdp)
{
	if (is_open())
	{
		...
		time32_type now = tick_now();
		double remoteToLocalLostrate = std::max(remote_to_local_lost_rate_, 0.0);

		packet<reliable_packet_format_type> writer;
		writer.set_control(CTRL_PUNCH);
		writer.set_peer_id(m_flow_id);
		//TODO:
...
void urdp_flow::on_received_punch_request(const safe_buffer& buf)
{
	...
	DEBUG_SCOPE(
		printf("------ on_received_punch_request: %s \n", endpoint_to_string(remote_edp).c_str());
	);
	error_code ec;
	if (m_shared_layer&&remote_edp.port())
	{
		for (int i = 0; i < 2; ++i)//send 2 times
		{
			m_shared_layer->async_send_to(
...
void urdp_flow::__on_clock(VC9_BIND_BUG_PARAM)
{
	...
	// Do not care about haveSentReliableMsg!!
...
endpoint urdp_flow::local_endpoint(error_code& ec)const
{
	if (is_open())
		return m_shared_layer->local_endpoint(ec);
	ec = asio::error::not_socket;
	return endpoint();
}

endpoint urdp_flow::remote_endpoint(error_code& ec)const
{
	if (is_open())
...
{
	if (!m_shared_layer || m_state == CLOSED)
	{
		__to_closed_state();
		return;
	}
...
{
	shared_layer_impl_sptr sharedLayer = m_shared_layer;
	uint32_t flowID = m_flow_id;
	...
	if (m_self_holder)
	{
		struct flow_unregister{
			static void go(shared_layer_impl_sptr sharedLayer, uint32_t flowID, boost::shared_ptr<urdp_flow>h){
				if (sharedLayer)
					sharedLayer->unregister_flow(flowID, h.get());
				BOOST_ASSERT(!h->m_self_holder);
			};
		};
		get_io_service().post(make_alloc_handler(boost::bind(&flow_unregister::go, sharedLayer, flowID, m_self_holder)));
	...
	if (__clock_check(now, t) && !b_timer_posted_)
	{
		if (t <= 0)
		{
			...
			else
			{
				...
				get_io_service().post(make_alloc_handler(
					boost::bind(&this_type::__on_clock, SHARED_OBJ_FROM_THIS VC9_BIND_BUG_PARAM_DUMMY)
					));
			}
		}
		else
	...
	boost::logic::tribool reliable, error_code& ec)
{
	...
	if (reliable)//reliable
	{
		...
		__queue(header, 4, CTRL_DATA, bufLen);
	...
	else//semireliable&unreliable
	{
		...
		else//semireliable
		{
			...
			if (lostRate > 0.5)
			{
				seg.timeout = now + random(20, 40);//resend
				seg.remainXmit = 4;
			}
			if (lostRate > 0.15)
			{
...
uint32_t urdp_flow::__queue(const char * data, size_t len, uint8_t ctrlType,
	size_t reserveLen)
{
	BOOST_ASSERT(m_self_holder);

	bool checkCombine = true;
	size_t writeLen = 0;
	while (len - writeLen > 0)
	{
		SSegment* backSegment = NULL;
		// We can concatenate data if the last segment is the same type
		// (control v. regular data), and has not been transmitted yet
		if (checkCombine
			&& ctrlType == CTRL_DATA
			&& !m_slist.empty()
			&& (backSegment = &m_slist.back(), backSegment->xmit == 0)
			&& (backSegment->ctrlType == ctrlType)
			&& (backSegment->buf.size() < m_mss)
			//&& (m_slist.back().buf.size()-m_mss>8)//
			)
		{
			safe_buffer& bf = backSegment->buf;
			size_t canWriteLen = std::min(m_mss - bf.size(), len - writeLen);
		...
		{
			checkCombine = false;
			size_t canWriteLen = std::min(m_mss, len - writeLen);
			size_t newBufferLen = canWriteLen;
...
int urdp_flow::__packet_as_unreliable_and_sendout(SUnraliableSegment& seg, time32_type now)
{
	...
	urdp_header.set_peer_id(seg.control == CTRL_CONNECT ? m_flow_id : m_remote_peer_id);
	...
	error_code ec;
	m_shared_layer->async_send_to(bufVec, m_remote_endpoint, ec);

	out_speed_meter_ += (seg.buf.length() + format_size);

	seg.remainXmit--;
	m_t_lasttraffic = m_t_lastsend = now;
	seg.timeout = now + random(20, 40);
	...
	const safe_buffer* data, time32_type now)
{
	...
	urdp_header.set_peer_id(control == CTRL_CONNECT ? m_flow_id : m_remote_peer_id);
	//TODO:
	//h.set_bandwidth_recving();
	urdp_header.set_lostrate_recving(uint32_t(remoteToLocalLostrate / LOST_RATE_PRECISION));
	urdp_header.set_id_for_lost_detect(id_for_lost_rate_++);
	...
	{
		m_shared_layer->async_send_to(urdp_header.buffer(),
...
bool urdp_flow::__process(const safe_buffer& sbuf, const endpoint& from)
{
	if (m_state == CLOSED)
	{
		BOOST_ASSERT(!m_self_holder);
	...
	time32_type now = tick_now();
	do{
		const size_t unreliable_format_size
	...
	uint32_t control = urdp_header.get_control();
	switch (control)
	{
	...
	case CTRL_CONNECT:
		...
		if (m_state == LISTEN)
		{
			...
			write_int32_hton(m_flow_id, pMyPeerID);
...
void urdp_flow::__update_rtt(long rtt)
{
	if (rtt >= 0)
	{
...
void urdp_flow::__attempt_send(SendFlags sflags)
{
	...
	while (true)
	{
		...
		if (m_slist.empty())
		{
			...
			if ((sflags == sfImmediateAck) || m_t_ack&&ACK_DELAY >= mod_minus(now, m_t_ack))
			{
				__packet_as_reliable_and_sendout(m_snd_nxt, CTRL_ACK, NULL, now);
			}
			else if (sflags != sfNone && m_t_ack == 0)
			{
	...
	wrappable_integer<int8_t>* id) const
{
	...
	while ((itr = qm.begin()) != end
		&& (
		qm.size() >= (size_t)(seqno_mark_type::max_distance() - 1)
