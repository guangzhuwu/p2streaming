...
trdp_flow::shared_layer_sptr trdp_flow::create_shared_layer_for_acceptor(
	io_service& ios, 
	endpoint& local_edp, 
...
{
	shared_layer_impl_sptr obj = shared_layer_impl_type::create(ios, local_edp, ec, realTimeUsage);
	if (ec)
		return boost::shared_ptr<basic_shared_layer_adaptor>();
	obj->register_acceptor(acceptor, ec);
	local_edp = obj->local_endpoint(ec);
	return boost::static_pointer_cast<shared_layer_type>(obj);
}

trdp_flow::trdp_flow(io_service& ios, bool realTimeUtility, bool passiveMode)
	:basic_engine_object(ios)
	, socket_impl_(ios)
	, b_passive_(passiveMode)
...
trdp_flow::~trdp_flow()
{
	boost::shared_ptr<shared_layer_type> sharedLayer = shared_layer_.lock();
	if (sharedLayer)
		sharedLayer->unregister_flow(flow_id_, this);
...
void trdp_flow::__init()
{
	...
	srtt_ = 0;
	flow_id_ = random<uint32_t>(0, INVALID_FLOWID);
	connection_ = NULL;
	sending_ = false;
	...
	else
	{
		...
		ping_timer_->register_time_handler(boost::bind(&this_type::ping, this));//bind signal
	}
}
...
double trdp_flow::__remote_to_local_lost_rate(int8_t* id)const
{
	...
	seqno_mark_map::iterator itr = qm.begin();
	seqno_mark_map::reverse_iterator ritr = qm.rbegin();
	while (!qm.empty()
		&& (
		qm.size() >= (std::size_t)(seqno_mark_type::max_distance() - 1)
		|| itr->second + 3000 < now//3000ms
		|| ((int)qm.size() > 1 && MARK_MINUS(ritr->first, itr->first) <= 0)
		|| (id&&MARK_MINUS(*id, itr->first) < 0)//mark_seq is ordered, for reason of TCP
		|| (id&&MARK_GREATER_EQUAL(itr->first, *id) && MARK_GREATER_EQUAL(*id, ritr->first))
		)
...
{
	shared_layer_ = boost::static_pointer_cast<shared_layer_impl_type>(shared_layer);

	//start time out timer
	conn_timer_->register_time_handler(boost::bind(&this_type::close, this, true));
	...
	op_stamp_t stamp, coroutine coro)
{
	...
	CORO_REENTER(coro)
	{
		...
		else
		{
			...
			shared_layer_impl_sptr sharedLayer = shared_layer_.lock();
			if (sharedLayer)
			{
	...
	const std::string& domainName)
{
	...
	else
	{
		...
		connect(endpoint(), domainName);
	}
}

void trdp_flow::connect(const endpoint& remote_edp, const std::string& domainName)
{
	error_code ec;
	...
	resolver_query* qy)
{
	...
	CORO_REENTER(coro)
	{
		// Start the conn timer
		{
			...
			conn_timer_->register_time_handler(
				boost::bind(&this_type::__to_close_state, this, timeoutErr, stamp)
				);
		...
		while (err && itr != resolver_iterator())
		{
			...
			{
				error_code timeoutErr = asio::error::timed_out;
				conn_timer_->register_time_handler(boost::bind(&this_type::__to_close_state, 
		...
		conn_timer_->cancel();
		conn_timer_->unregister_time_handler();
	...
	op_stamp_t stamp, coroutine coro)
{
	...
	CORO_REENTER(coro)
	{
		...
		conn_timer_->register_time_handler(
			boost::bind(&this_type::__to_close_state, this, ec, stamp)
			);
		conn_timer_->async_wait(seconds(6));
...
error_code trdp_flow::__open(error_code& ec)
{
	...
	if (!ec)
	{
		...
		if (is_realtime_utility_)
		{
			...
			asio::socket_base::send_low_watermark send_low_watermark_option(16);
			asio::socket_base::receive_low_watermark receive_low_watermark_option(16);
	...
	uint16_t* msgType, bool fromQue)
{
	...
	if ((!send_bufs_.empty() && !fromQue) || sending_)
	{
		...
		send_bufs_.push_back(elm);

		if (send_bufs_.size() > 1024)
		{
			//printf("??\n");
		}
