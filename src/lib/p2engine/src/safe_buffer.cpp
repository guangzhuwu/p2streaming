...
namespace p2engine{
	...
	void safe_buffer_base::reserve(size_t len)
	{
		if (len==0)
			return;
		if (capacity()<len)
	...
	void safe_buffer_base::realloc(size_t len)
	{
		if (len==0)
		{
			reset();
	...
	{
		if (buffer_length == (std::numeric_limits<size_t>::max)())
	...
	size_t safe_buffer_base::memset(uint8_t byte_val, 
		size_t len , 
		size_t byte_offset)
	{
