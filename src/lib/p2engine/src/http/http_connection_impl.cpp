...
namespace p2engine { namespace http { namespace detail {
		...
		)
	{
			...
			&&life_time != boost::date_time::neg_infin)
		{
			...
			life_timer_->register_time_handler(boost::bind(&this_type::__to_close_state, 
		...
		)
	{
		...
		else
		{
			...
			async_connect(endpoint(), time_out);
		}
	}
		...
		)
	{
		...
		{
			error_code timeoutErr = asio::error::timed_out;
			conn_timeout_timer_->register_time_handler(
				boost::bind(&this_type::__to_close_state, 
				this, timeoutErr, op_stamp()));
	...
	error_code basic_http_connection_impl::__open(const endpoint& local_edp, 
		error_code& ec)
	{
		__init();
		if (state_ != INIT)
		...
		)
	{
		...
		CORO_REENTER(coro)
		{
			...
			if (resolver_query_)
			{
				...
				resolve_timer_->register_time_handler(boost::bind(
	...
	void basic_http_connection_impl::__retry_async_resolve_connect_coro(op_stamp_t stamp)
	{
		...
		conn_retry_timer_->cancel();
		conn_retry_timer_->register_time_handler(
			boost::bind(&this_type::__async_resolve_connect_coro, 
	...
	void basic_http_connection_impl::__async_send_handler(
		error_code ec, size_t len, op_stamp_t stamp)
	{
		//all canceled operation will not be invoked
		if (state_ != CLOSING && (is_canceled_op(stamp) || state_ == CLOSED))
			return;
		...
		size_t len, op_stamp_t stamp)
	{
		...
		if (!is_header_recvd_)
		{
			http::request  req;
			http::response res;
			http::header* h = (b_passive_ ? (http::header*)(&req) : (http::header*)(&res));
			h->clear();
			...
			if (parseRst > 0)
			{
				...
				safe_buffer buf;
				if (size_t(parseRst) < recv_buf_.size())
				{
	...
	{
		if (connection_)
			connection_->dispatch_message(data);
	}
