...
namespace p2engine {
	...
	class safe_buffer_base
	{
		...
		BOOST_STATIC_CONSTANT(size_t, BUFFER_DELTA=128);
	public:
		safe_buffer_base():p_ptr_(NULL), g_ptr_(NULL)
		{}
		explicit safe_buffer_base(size_t len)
			: raw_buffer_(raw_buffer::create(((std::max))(len, (size_t)BUFFER_DELTA)))
		{
			gptr(raw_buffer_ptr());
		...
		safe_buffer_base(const void* data, size_t data_len)
			: raw_buffer_(raw_buffer::create(((std::max))(data_len, (size_t)BUFFER_DELTA)))
		{
			gptr(raw_buffer_ptr());
		...
		safe_buffer_base buffer_ref(std::ptrdiff_t byte_offset, 
			size_t buffer_length = (std::numeric_limits<size_t>::max)());
		...
		void reset(const safe_buffer_base &buf, size_t trucated_len = (std::numeric_limits<size_t>::max)())
		{
			if (this!=&buf)
			{
		...
		size_t length() const 
		{
			if (!raw_buffer_)
				return 0;
			BOOST_ASSERT(raw_buffer_len() >=(size_t)(p_ptr_ - g_ptr_));
		...
		template <typename T>
		void set(T val, size_t byte_offset = 0)
		{
			BOOST_ASSERT(this->raw_buffer_ != NULL);
			BOOST_ASSERT(gptr() + byte_offset + sizeof(T) <= pptr());
		...
		size_t get_data(safe_buffer_base &buf, size_t len, size_t byte_offset = 0) const
		{
			return get_data(buf.gptr(), buf.length(), len, byte_offset);
		}
		void set_data(const safe_buffer_base& buf, size_t len, size_t byte_offset = 0)
		{
		...
		size_t memset(uint8_t byte_val, size_t len = (std::numeric_limits<size_t>::max)(), 
			...
			size_t bytes_per_line = static_cast<size_t>(0x10)) const
		{
			...
			else
			{
				const char* p=gptr();
				os <<"<<"<<(ptrdiff_t)p<<">>\n";
				BOOST_ASSERT(trucated_len == (std::numeric_limits<size_t>::max)() ||
	...
	public:
		...
		asio_const_buffer(const asio_const_buffer &buf)
			: boost::asio::const_buffer(static_cast<const boost::asio::const_buffer&>(buf)), 
	...
	public:
		...
		safe_buffer(size_t len) : safe_buffer_base(len) {};
		safe_buffer(const safe_buffer_base &buf, 
			size_t trucated_len = (std::numeric_limits<size_t>::max)())
			: safe_buffer_base(buf, trucated_len) {};
		safe_buffer(const void* data, size_t data_len )
		...
		{
			BOOST_ASSERT(raw_buffer_);
			BOOST_ASSERT(trucated_len == (std::numeric_limits<size_t>::max)() || trucated_len <= length());
			return asio_const_buffer(*this, (trucated_len == (std::numeric_limits<size_t>::max)()) ? length() : trucated_len);
		};
		asio_mutable_buffer to_asio_mutable_buffer(size_t trucated_len = (std::numeric_limits<size_t>::max)()) const
		{
			BOOST_ASSERT(raw_buffer_);
			BOOST_ASSERT(trucated_len == (std::numeric_limits<size_t>::max)() || trucated_len <= length());
			return asio_mutable_buffer(*this, (trucated_len == (std::numeric_limits<size_t>::max)()) ? length() : trucated_len);
		};
		asio_const_buffers_1 to_asio_const_buffers_1(size_t trucated_len = (std::numeric_limits<size_t>::max)()) const
		{
			BOOST_ASSERT(raw_buffer_);
			BOOST_ASSERT(trucated_len == (std::numeric_limits<size_t>::max)() || trucated_len <= length());
			return asio_const_buffers_1(*this, (trucated_len == (std::numeric_limits<size_t>::max)()) ? length() : trucated_len);
		};
		asio_mutable_buffers_1 to_asio_mutable_buffers_1(size_t trucated_len = (std::numeric_limits<size_t>::max)()) const
		{
			BOOST_ASSERT(raw_buffer_);
			BOOST_ASSERT(trucated_len == (std::numeric_limits<size_t>::max)() || trucated_len <= length());
			return asio_mutable_buffers_1(*this, (trucated_len == (std::numeric_limits<size_t>::max)()) ? length() : trucated_len);
		};
	};
