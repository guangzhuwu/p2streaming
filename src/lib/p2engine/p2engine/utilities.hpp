...
namespace p2engine {
	...
	template <class OStream, size_t FieldWidth, typename T, bool IS_INTEGRAL>
	struct stream_then_format_if_integral;

	template <class OStream, size_t FieldWidth, typename T>
	...
	template <class OStream, typename T, size_t TypeSize>
	...
	struct stream_then_format_if_integral<OStream, FieldWidth, T, true>
	{
		...
		{
			BOOST_STATIC_ASSERT(boost::is_integral<T>::value);
			os.width(FieldWidth - sizeof(T) * static_cast<size_t>(2)); os.fill(' '); os <<' ';
			os.width(sizeof(T) * static_cast<size_t>(2)); os.fill('0');
