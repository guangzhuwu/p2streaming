...
namespace p2engine {
	...
	public:
		...
		size_t write(const std::string& str, size_t len)
		{
		...
		size_t read(std::string& str, size_t len)
		{
			len=(std::min)(len, buffer_.size());
		...
		size_t write(const safe_buffer& buf, size_t len)
		{
			len=(std::min)(len, buf.size());
			return write(buffer_cast<const char*>(buf), len);
		}
		size_t write(const safe_buffer& buf)
		{
			return write(buffer_cast<const char*>(buf), buf.length());
		}
		size_t read(safe_buffer& buf, size_t len)
		{
			len=(std::min)(len, buffer_.size());
			uint8_t* p=(uint8_t*)buffer_.gptr();
			if (len==0)
