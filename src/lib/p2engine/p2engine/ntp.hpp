...
namespace p2engine{
	...
	public:
		typedef boost::function<void(const ptime&, const error_code& ec)> on_gottime_handle_type;
		...
		on_gottime_handle_type& gottime_handle(){return signal_;}
		const on_gottime_handle_type& gottime_handle()const{return signal_;}
	...
	private:
		...
		udp::socket socket_;
		on_gottime_handle_type signal_;
		char buf_[48];
