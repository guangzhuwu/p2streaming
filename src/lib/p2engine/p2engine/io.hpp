...
namespace p2engine{
	...
	inline T read_value(InIterator& start, bool advance=true, size_t bytesize=sizeof(T))
	{
	...
	inline void write_value(T val, OutIterator& start, bool advance=true, size_t bytesize=sizeof(T))
	{
	...
	template <typename T, typename InIterator>
	inline T read_int(InIterator& start, bool advance=true, size_t bytesize=sizeof(T))
	{
		BOOST_STATIC_ASSERT(boost::is_float<T>::value||boost::is_integral<T>::value);
		return read_value<T>(start, advance, bytesize);
	}
	template <typename T, typename InIterator>
	inline T read_int_ntoh(InIterator& start,  bool advance=true, size_t bytesize=sizeof(T))
	{
		return ntoh<T>()(read_int<T, InIterator>(start, advance, bytesize));
	}

	template <typename T, typename OutIterator>
	inline void write_int(T val, OutIterator& start, bool advance=true, size_t bytesize=sizeof(T))
	{
		BOOST_STATIC_ASSERT(boost::is_float<T>::value||boost::is_integral<T>::value);
	...
	template <typename T, typename OutIterator>
	inline void write_int_hton(T val, OutIterator& start, bool advance=true, size_t bytesize=sizeof(T))
	{
