...
// ~~~~~~~~~~~~~~~~~~~~~~~~~
//
// Copyright (c) 2008-2008 Meng Zhang
//
...
// the endianness issue if the field is arithmetic, 
// the data is always stored in network byte order if it is arithmetic data
// (i.e. all build-in C++ 8, 16, 32, 64-bit integral and floating point types), 
// no matter on what platform (any big/little endian CPU)
...
// you do not have to serialize/deserialize the packet manually, 
// and do not have to write code to compute/change/update the offset 
// of the field in packet and the packet length.
//
...
//
// 5. With integrated with p2net::safe_buffer, 
...
//    P2ENGINE_PACKET_FIELD_DEF (uint16_t, src_port)
//    P2ENGINE_PACKET_FIELD_DEF (uint16_t, dst_port)
//    P2ENGINE_PACKET_FIELD_DEF (uint32_t, seq_number)
//    P2ENGINE_PACKET_FIELD_DEF_INIT (uint32_t, ack_number, 0xeeff)
//    P2ENGINE_PACKET_BIT_FIELD_DEF (uint8_t, data_offset, 4)
//    P2ENGINE_PACKET_BIT_FIELD_DEF (uint8_t, reserved1,    4)
//    P2ENGINE_PACKET_BIT_FIELD_DEF (uint8_t, reserved2,    1)
//    P2ENGINE_PACKET_BIT_FIELD_DEF (uint8_t, reserved3,    1)
//    P2ENGINE_PACKET_BIT_FIELD_DEF (uint8_t, URG, 1)
//    P2ENGINE_PACKET_BIT_FIELD_DEF (uint8_t, ACK, 1)
//    P2ENGINE_PACKET_BIT_FIELD_DEF (uint8_t, PSH, 1)
//    P2ENGINE_PACKET_BIT_FIELD_DEF_INIT    (uint8_t, RST, 1, 0)
//    P2ENGINE_PACKET_BIT_FIELD_DEF (uint8_t, SYN, 1)
//    P2ENGINE_PACKET_BIT_FIELD_DEF (uint8_t, FIN, 1)
//    P2ENGINE_PACKET_FIELD_DEF (uint16_t, win_size)
//    P2ENGINE_PACKET_FIELD_DEF (uint16_t, check_sum)
//    P2ENGINE_PACKET_FIELD_DEF (uint16_t, urgent_ptr)
//    P2ENGINE_PACKET_BIT_FIELD_DEF (uint32_t, options, 24)
//    P2ENGINE_PACKET_BIT_FIELD_DEF (uint32_t, padding, 8)
...
// and the buffer size can be either more than header.format_size()
// or less than header.format_size().
// If the buffer size is less then header.format_size()
...
//
// tcp_header::header_type(const p2net::safe_buffer& buf, bool init = true);
//
...
//
// uint16_t tcp_header::header_type::get_src_port() const;
// void tcp_header::header_type::set_src_port(const uint16_t &field);
//
...
//
// static size_t tcp_header::header_type::format_size();
// virtual size_t packet_size() const;
//
// that is, you can use the following code to get the declared packet size:
//
// size_t packet_sz = header.format_size();
// or
// size_t packet_sz = tcp_header::header_type::format_size();
// or
// size_t packet_sz = header.packet_size();
//
//
// 4. Get buffer (e.g.):
//
// the following member function is automatically declared and defined:
//
// p2net::safe_buffer tcp_header::header_type::buffer() const;
//
// that is, you can use the following code to get the buffer of the packet:
//    p2net::safe_buffer buf = header.buffer();
//
...
//        size_t max_dumped_bytes = (std::numeric_limits<size_t>::max)());
...
// the following member functions are automatically declared and defined:
//
// size_t tcp_header::header_type::bit_offset_of_src_port() const;
// size_t tcp_header::header_type::byte_offset_of_src_port() const;
//    size_t bits_of_src_port() const;
//    size_t bytes_of_src_port() const;
//
// that is, you can use the following code to dump all the fields in packet to your stream:
//
//    size_t bit_offset = header.bit_offset_of_src_port();
//    size_t byte_offset = header.byte_offset_of_src_port();
//    size_t bits = header.bits_of_src_port();//return 16 (bit)
//    size_t bytes= header.bytes_of_src_port();//return 2 (bytes)
//
// Note that for bit field, the ``bytes_of_xxx" returns the bytes of shared_ptr defined in macro:
// e.g. with
//    P2ENGINE_PACKET_BIT_FIELD_DEF    (uint8_t,    ACK,        1    )
// defined in Instruction 1, 
//
//    size_t bits = header.bits_of_ACK();//return 1 (bit)
//    size_t bytes= header.bytes_of_ACK();//return 1 (byte)
//
// 7. you can replace P2ENGINE_PACKET_FIELD_DEF to P2ENGINE_PACKET_FIELD_DEF_INIT
// so that the field can be initialized when the packet is created
//    e.g., 
// P2ENGINE_PACKET_FIELD_DEF_INIT(uint16_t, src_port, 0xFF)
//
// 8. To initialize bit field, when packet is created
// please use P2ENGINE_PACKET_BIT_FIELD_DEF_INIT instead
// e.g., 
// P2ENGINE_PACKET_BIT_FIELD_DEF_INIT    (uint8_t,    RST,        0    )
//
// 9. You can also define array and optionally with initialization
// please use P2ENGINE_PACKET_ARRAY_FIELD_DEF instead
// e.g., 
//    P2ENGINE_PACKET_ARRAY_FIELD_DEF        (long, ext3, 3    )//3 units with shared_ptr ``long" is declared
//    P2ENGINE_PACKET_ARRAY_FIELD_DEF_INIT        (long, ext4, 5, 0xAF    )//5 units with shared_ptr ``long" is declared and are all initialized with value 0xAF
//
...
//        {set_src_port(0xAA);})
//    P2ENGINE_PACKET_FIELD_DEF_INIT (uint32_t, extended_field1, 0xAAA)
//    P2ENGINE_PACKET_FIELD_DEF (uint8_t, extended_field2)
...
// the following function members are automatically declared and defined
// to get and set the buffer
//
// size_t tcp_header_extended::header_type::get_payload(void * buf, size_t buf_size, size_t len);
// size_t tcp_header_extended::header_type::get_payload(p2net::safe_buffer &buf, size_t len);
// void tcp_header_extended::header_type::set_payload(const void * buf, size_t len);
// void tcp_header_extended::header_type::set_payload(const p2net::safe_buffer& buf, size_t len);
//
// 13. A function truncated_size() is defined for each class
// with default value equal to packet_size() or format_size() of that packet.
...
// to the packet_size() minus the unset length of the buffer, 
// that is, 
// e.g., in Instruction 10, we define the last field as, 
// P2ENGINE_PACKET_BUF_FIELD_DEF (payload, 1500)
//
...
//
// size_t len = ext_header.truncated_size();//returned value is equal to ``ext_header.packet_size()"
//
...
//
// size_t new_len = ext_header.truncated_size();//returned value is equal to ``ext_header.packet_size() - 300"
//
// ``new_len" will be ``ext_header.packet_size() - 300"
//
...
// You can defined a field by a predefined packet field by macros
...
//        P2ENGINE_PACKET_FIELD_DEF_INIT (uint8_t, nfield1, 0xA8)
//        P2ENGINE_PACKET_FORMAT_DEF_END
//};
//
...
//        P2ENGINE_PACKET_FIELD_DEF_INIT (uint8_t, field1, 0xA1)
...
//
// assert(fields.get_field3().get_nested_field4() == 0x09);
...
protected: \
	typedef boost::uint_t<SIGNATURE_BITS>::least signature_type; \
	static const size_t SIG_BYTES = SIGNATURE_BITS?sizeof(signature_type):0; \
	static const size_t SIG_BITS  = SIGNATURE_BITS; \
public: \
	PACKET_FORMAT_DEF() \
	{ \
	}; \
	...
	p2engine::packet_format_def_base() \
	{ \
	reset(buf, init); \
	}; \
	\
	PACKET_FORMAT_DEF(const PACKET_FORMAT_DEF& packet_format) : \
	buffer_(packet_format.buffer_), truncated_size_(packet_format.truncated_size_) \
	{ \
	}; \
	\
	virtual ~PACKET_FORMAT_DEF() {}; \
	\
	void reset(const p2engine::safe_buffer& buf, bool init = true) \
	{ \
	buffer_ = buf; \
	if (init) \
		{ \
		set_signature(); \
		truncated_size_ = PACKET_FORMAT_DEF::format_size(); \
		PACKET_FORMAT_DEF::initialize(); \
		user_init(); \
		} \
	}; \
	\
	const p2engine::safe_buffer& buffer() const {return buffer_;}; \
	p2engine::safe_buffer& buffer(){return buffer_;}; \
	\
	...
	\
	virtual size_t packet_size() const {return PACKET_FORMAT_DEF::format_size();}; \
	\
	virtual size_t truncated_size() const {return truncated_size_;}; \
	\
...
protected: \
	...
	static const size_t PACKET_BASE_BYTES = SIG_BYTES; \
	\
...
protected: \
	...
	static const size_t PACKET_BASE_BYTES = PACKET_FORMAT_DEF_BASE::PKT_SIZE; \
	\
...
public: \
	\
public: \
	PACKET_FORMAT_DEF() : PACKET_FORMAT_DEF_BASE() \
	{ \
	}; \
	\
	PACKET_FORMAT_DEF(const p2engine::safe_buffer& buf, bool init = true) : \
	PACKET_FORMAT_DEF_BASE(buf, init) \
	{ \
	__reset(buf, init); \
	}; \
	\
	...
	void reset(const p2engine::safe_buffer& buf, bool init = true) \
	{ \
	PACKET_FORMAT_DEF_BASE::reset(buf, init); \
	__reset(buf, init); \
	}; \
	\
private: \
	\
	void __reset(const p2engine::safe_buffer& buf, bool init = true) \
	{ \
	if (init) \
		{ \
...
public: \
	...
	virtual size_t packet_size() const {return PACKET_FORMAT_DEF::format_size();}; \
	\
	virtual size_t truncated_size() const \
	{ \
	...
	size_t bit_offset_base = 0) const \
	{ \
...
#define P2ENGINE_PACKET_FORMAT_DEF_BEGIN_COMMON \
	\
public: \
	const static std::string& format_signature_str() \
	{ \
	static std::string s_signature_str; \
	if (s_signature_str.empty()) \
	...
	static signature_type s_signature=\
	static_cast<signature_type>(p2engine::get_str_hash_value<SIG_BITS>()(format_signature_str())); \
	return s_signature; \
	}; \
	\
protected: \
...
struct __dumper_begin \
	...
	size_t max_dumped_bytes, \
	const String& field_name_prefix, \
	size_t bit_offset_base) {return os;}; \
	}; \
...
public: \
	...
	size_t bytes_of_##FIELD_NAME() const \
	{ \
	return sizeof(TYPE); \
	}; \
	size_t bits_of_##FIELD_NAME() const \
	{ \
	return sizeof(TYPE) * static_cast<size_t>(8); \
	};
...
protected: \
	...
	\
	static const size_t ABS_BIT_OFFSET_OF_##FIELD_NAME = PACKET_BASE_BYTES * 8 \
	+ head_offset_##FIELD_NAME::FIELD_HEAD_BIT_OFFSET; \
	static const size_t ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME = ABS_BIT_OFFSET_OF_##FIELD_NAME / 8;\
	\
struct dumper_##FIELD_NAME : public rear_offset_##FIELD_NAME::prev_dumper \
	...
	size_t bit_offset_base) \
		...
		(std::min)(buf.length(), max_dumped_bytes)) \
			...
			p2engine::stream_to_text<OStream, FIELD_WIDTH_IN_TEXT, TYPE>()( \
			os, \
			p2engine::ntoh<TYPE>() \
			(buf.get<TYPE>(ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME)) \
			); \
			os <<std::setw(15)<<" | head bit offset: " \
...
public: \
	...
	\
	static size_t byte_offset_of_##FIELD_NAME() \
	{return ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME;}; \
	\
	static size_t bit_offset_of_##FIELD_NAME() \
...
public: \
	TYPE get_##FIELD_NAME() const \
	{ \
	return p2engine::get_bit_field<TYPE>()(buffer_, ABS_BYTE_OFFSET - OFFSET_IN_TYPE/8, OFFSET_IN_TYPE, BITS); \
	}; \
	\
	void set_##FIELD_NAME(const TYPE &field) \
	{ \
	P2ENGINE_BIT_FIELD_ASSIGN(FIELD_NAME, field) \
	p2engine::set_bit_field<TYPE>()(buffer_, ABS_BYTE_OFFSET - OFFSET_IN_TYPE/8, OFFSET_IN_TYPE, BITS, field); \
	}; \
	size_t bytes_of_##FIELD_NAME() const \
	{ \
	return sizeof(TYPE); \
...
#define P2ENGINE_PACKET_BIT_FIELD_DEF_DETAIL_COMMON(TYPE, FIELD_NAME, BITS, GET_AND_SET_BIT_FIELD_MEMBERS) \
	\
	static const size_t ABS_BIT_OFFSET_OF_##FIELD_NAME \
	= PACKET_BASE_BYTES * 8 + \
	rear_offset_##FIELD_NAME::FIELD_REAR_BIT_OFFSET - static_cast<size_t>(BITS); \
	static const size_t ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME = ABS_BIT_OFFSET_OF_##FIELD_NAME / 8; \
	static const size_t BIT_OFFSET_IN_CUR_TYPE_##FIELD_NAME \
	= rear_offset_##FIELD_NAME::FIELD_REAR_BIT_OFFSET - static_cast<size_t>(BITS) \
	- head_offset_##FIELD_NAME::FIELD_HEAD_BIT_OFFSET; \
	\
struct check_bits_of_##FIELD_NAME \
	{ \
	BOOST_STATIC_ASSERT(static_cast<size_t>(BITS) <= p2engine::type_bits<TYPE>::TYPE_BITS \
	&& BIT_OFFSET_IN_CUR_TYPE_##FIELD_NAME >= static_cast<size_t>(0) \
...
struct dumper_##FIELD_NAME : public rear_offset_##FIELD_NAME::prev_dumper \
	...
	size_t bit_offset_base) \
		...
		(std::min)(buf.length(), max_dumped_bytes)) \
			...
			p2engine::stream_to_text<OStream, FIELD_WIDTH_IN_TEXT, TYPE>()( \
			os, \
			p2engine::get_bit_field<TYPE>()(buf, ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME, BIT_OFFSET_IN_CUR_TYPE_##FIELD_NAME, BITS)\
			); \
			os <<std::setw(15)<<" | head bit offset: " \
			<<std::setw(5)<<std::dec<<bit_offset_base + ABS_BIT_OFFSET_OF_##FIELD_NAME \
			<<std::endl; \
...
public: \
	TYPE get_##FIELD_NAME(size_t idx) const \
	{ \
	assert(idx < static_cast<size_t>(ARR_SIZE)); \
	...
	\
	void set_##FIELD_NAME(size_t idx, const TYPE &field) \
	{ \
	...
	size_t bytes_of_##FIELD_NAME() const \
	{ \
	return sizeof(TYPE) * static_cast<size_t>(ARR_SIZE); \
	}; \
	size_t bits_of_##FIELD_NAME() const \
	{ \
	return bytes_of_##FIELD_NAME() * static_cast<size_t>(8); \
	};
...
public: \
	...
	ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME); \
	\
	static size_t byte_offset_of_##FIELD_NAME(size_t idx = 0) \
	{return ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + idx;}; \
	\
	static size_t bit_offset_of_##FIELD_NAME(size_t idx = 0) \
	{return ABS_BIT_OFFSET_OF_##FIELD_NAME \
...
protected: \
	...
	head_offset_##FIELD_NAME::FIELD_HEAD_BIT_OFFSET, \
	rear_offset_##FIELD_NAME::FIELD_REAR_BIT_OFFSET, \
	p2engine::type_bits<TYPE>::TYPE_BITS * static_cast<size_t>(ARR_SIZE), 
...
class intlzr_##FIELD_NAME : public rear_offset_##FIELD_NAME::prev_initializer \
	...
	static void initialize(packet_format_type& packet_format) \
		...
		+ (i + static_cast<size_t>(1)) * sizeof(TYPE) <= packet_format.buffer().length()) \
				{ \
...
struct dumper_##FIELD_NAME : public rear_offset_##FIELD_NAME::prev_dumper \
	...
	template <class OStream, class String> \
	static OStream& dump(OStream& os, const p2engine::safe_buffer &buf, \
	size_t max_dumped_size, \
	const String& field_name_prefix, \
	size_t bit_offset_base) \
		{ \
		rear_offset_##FIELD_NAME::prev_dumper::dump(os, buf, \
		max_dumped_size, field_name_prefix, bit_offset_base); \
		if (ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + static_cast<size_t>(SIZE) <= \
...
public: \
	size_t get_##FIELD_NAME(void * buf, size_t buf_size, size_t len = (std::numeric_limits<size_t>::max)()) const \
	{ \
	size_t true_len = (std::min)(static_cast<size_t>(SIZE), len); \
	return buffer_.get_data(buf, buf_size, true_len, ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME); \
	}; \
	\
	void set_##FIELD_NAME(const void * buf, size_t len) \
	{ \
	assert(len <= static_cast<size_t>(SIZE)); \
	P2ENGINE_BUF_FIELD_ASSIGN(FIELD_NAME, SIZE, buf, len) \
	buffer_.set_data(buf, len, ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME); \
	if (ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + static_cast<size_t>(SIZE) == this->packet_size()) \
		{ \
		truncated_size_ = ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + len; \
		} \
	}; \
	size_t get_##FIELD_NAME(p2engine::safe_buffer& buf, size_t len = (std::numeric_limits<size_t>::max)()) const \
	{ \
	size_t true_len = (std::min)(static_cast<size_t>(SIZE), len); \
	buf = buffer_.buffer_ref(ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME, true_len).clone(); \
	return buf.length(); \
	}; \
	\
	void set_##FIELD_NAME(const p2engine::safe_buffer &buf, size_t len = (std::numeric_limits<size_t>::max)()) \
	{ \
	size_t true_len = (std::min)(buf.length(), len); \
	assert(true_len <= static_cast<size_t>(SIZE)); \
	P2ENGINE_BUF_FIELD_ASSIGNED_BY_SAFE_BUF(FIELD_NAME, SIZE, buf, true_len) \
	buffer_.set_data(buf, true_len, ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME); \
	if (ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + static_cast<size_t>(SIZE) == this->packet_size()) \
		{ \
		truncated_size_ = ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + len; \
		} \
...
#define P2ENGINE_PACKET_BUF_FIELD_DEF_DETAIL(FIELD_NAME, SIZE) \
	\
	static_cast<size_t>(SIZE * 8)> rear_offset_##FIELD_NAME; \
	\
	typedef p2engine::field_head_offset<rear_offset_##FIELD_NAME::PREV_FIELD_HEAD_BIT_OFFSET, \
	rear_offset_##FIELD_NAME::PREV_FIELD_REAR_BIT_OFFSET, true> head_offset_##FIELD_NAME; \
	\
...
#define P2ENGINE_PACKET_NESTED_FIELD_DEF_DETAIL(FORMAT_DEF, FIELD_NAME) \
	\
	static_cast<size_t>(FORMAT_DEF::PKT_SIZE * 8)> rear_offset_##FIELD_NAME; \
	\
	typedef p2engine::field_head_offset<rear_offset_##FIELD_NAME::PREV_FIELD_HEAD_BIT_OFFSET, \
	rear_offset_##FIELD_NAME::PREV_FIELD_REAR_BIT_OFFSET, true> head_offset_##FIELD_NAME; \
	\
...
class intlzr_##FIELD_NAME : public rear_offset_##FIELD_NAME::prev_initializer \
	...
	static void initialize(packet_format_type& packet_format) \
		...
		rear_offset_##FIELD_NAME::prev_initializer::initialize(packet_format); \
		if (ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + \
		FORMAT_DEF::format_size() <= packet_format.buffer().length()) \
			{ \
			FORMAT_DEF field \
...
struct dumper_##FIELD_NAME : public rear_offset_##FIELD_NAME::prev_dumper \
	...
	size_t bit_offset_base) \
		...
		max_dumped_size, field_name_prefix, bit_offset_base); \
		if (ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + \
		FORMAT_DEF::format_size() <= buf.length()) \
			{ \
			FORMAT_DEF field \
			(buf.buffer_ref(ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME, \
			FORMAT_DEF::format_size()), false); \
...
protected: \
	...
	head_offset_##FIELD_NAME::FIELD_HEAD_BIT_OFFSET, \
	rear_offset_##FIELD_NAME::FIELD_REAR_BIT_OFFSET, \
	FORMAT_DEF::PKT_SIZE * static_cast<size_t>(8), 
...
#define P2ENGINE_PACKET_NESTED_ARRAY_FIELD_DEF_DETAIL(FORMAT_DEF, FIELD_NAME, ARR_SIZE) \
	\
	static_cast<size_t>(FORMAT_DEF::PKT_SIZE * ARR_SIZE * 8)> rear_offset_##FIELD_NAME; \
	\
	typedef p2engine::field_head_offset<rear_offset_##FIELD_NAME::PREV_FIELD_HEAD_BIT_OFFSET, \
	rear_offset_##FIELD_NAME::PREV_FIELD_REAR_BIT_OFFSET, true> head_offset_##FIELD_NAME; \
	\
...
class intlzr_##FIELD_NAME : public rear_offset_##FIELD_NAME::prev_initializer \
	...
	static void initialize(packet_format_type& packet_format) \
		...
		for(size_t i = 0; i < static_cast<size_t>(ARR_SIZE); ++ i) \
			{ \
			if (ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + \
			(i + 1) * FORMAT_DEF::format_size() <= packet_format.buffer().length()) \
				{ \
				FORMAT_DEF field \
				(packet_format.buffer().buffer_ref(ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + \
				i * FORMAT_DEF::format_size(), FORMAT_DEF::format_size()), true); \
				} \
			} \
		}; \
		static void append_field_signature_str(std::string &str_to_append) \
...
struct dumper_##FIELD_NAME : public rear_offset_##FIELD_NAME::prev_dumper \
	...
	size_t bit_offset_base) \
		...
		for(size_t i = 0; i < static_cast<size_t>(ARR_SIZE); ++ i) \
			{ \
			if (ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + \
			(i + 1) * FORMAT_DEF::format_size() <= buf.length()) \
				{ \
				FORMAT_DEF field \
				(buf.buffer_ref(ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + \
				i * FORMAT_DEF::format_size(), FORMAT_DEF::format_size()), false); \
				field.dump(os, max_dumped_size, \
				field_name_prefix + boost::lexical_cast<String>(#FIELD_NAME "[") + \
				boost::lexical_cast<String>(i) + boost::lexical_cast<String>("]::"), \
				bit_offset_base + ABS_BIT_OFFSET_OF_##FIELD_NAME + \
				i * FORMAT_DEF::format_size()); \
				} \
			} \
			return os; \
		} \
...
public: \
	FORMAT_DEF get_##FIELD_NAME(size_t idx) const\
	{ \
	assert(ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + \
	(idx + 1) * FORMAT_DEF::format_size() <= buffer_.length()); \
	return FORMAT_DEF \
	(buffer_.buffer_ref(ABS_HEAD_BYTE_OFFSET_OF_##FIELD_NAME + \
	idx * FORMAT_DEF::format_size(), FORMAT_DEF::format_size()), false); \
	}; \
	\
	size_t bytes_of_##FIELD_NAME() const \
	{ \
...
protected: \
	...
	head_offset_##FIELD_NAME::FIELD_HEAD_BIT_OFFSET, \
	rear_offset_##FIELD_NAME::FIELD_REAR_BIT_OFFSET, \
	static_cast<size_t>(FORMAT_DEF::PKT_SIZE * ARR_SIZE * 8), 

//////////////////////////////////////////////////////////////////////////

#define P2ENGINE_PACKET_FORMAT_DEF_END \
	\
	static_cast<size_t>(p2engine::type_bits<p2engine::pkt_field_assist_type>::TYPE_BITS)> __field_offset_end; \
	\
public: \
	BOOST_STATIC_ASSERT(!START_WITH_DERIVE_BEGIN); \
	static const size_t PKT_SIZE = PACKET_BASE_BYTES + \
	__field_offset_end::PREV_FIELD_REAR_BIT_OFFSET / 8; \
	static size_t format_size() \
	{return PKT_SIZE;}; \
	\
	virtual std::ostream& dump(std::ostream& os, \
	size_t max_dumped_bytes = (std::numeric_limits<size_t>::max)(), \
...
# define P2ENGINE_FIELD_ARRAY_ASSIGN(FIELD_NAME, ARR_SIZE, idx, field) \
	assert(static_cast<size_t>(idx) >= 0 && static_cast<size_t>(idx) < static_cast<size_t>(ARR_SIZE)); \
	FIELD_NAME##_[idx] = field;

# define P2ENGINE_BUF_FIELD_DEF(FIELD_NAME, SIZE) uint8_t FIELD_NAME##_[SIZE];
# define P2ENGINE_BUF_FIELD_ASSIGN(FIELD_NAME, SIZE, buf, len) \
	assert(static_cast<size_t>(len) <= static_cast<size_t>(SIZE)); \
...
namespace p2engine {
	...
	public:
		virtual std::ostream& dump(std::ostream& os, 
			size_t max_dumped_bytes = (std::numeric_limits<size_t>::max)(), 
			const std::string& field_name_prefix = std::string(), 
			size_t bit_offset_base = 0) const = 0;
#        if !defined(P2ENGINE_NO_WIDE_FUNCTIONS)
		virtual std::wostream& dump(std::wostream& os, 
			size_t max_dumped_bytes = (std::numeric_limits<size_t>::max)(), 
			const std::wstring& field_name_prefix = std::wstring(), 
...
#        endif
	...
	OStream& operator<<(OStream& os, const packet_format_def_base& packet_field)
	{
		return static_cast<OStream&>(packet_field.dump(os));
	}

	template<typename INITIALIZER, typename DUMPER, 
		size_t PREV_HEAD, size_t PREV_REAR, size_t PREV_TYPE_BITS, size_t BITS>
	struct field_rear_offset
	{
	public:
		static const size_t PREV_FIELD_HEAD_BIT_OFFSET = PREV_HEAD;
		static const size_t PREV_FIELD_REAR_BIT_OFFSET = PREV_REAR;
		static const size_t PREV_FIELD_TYPE_BITS = PREV_TYPE_BITS;
		static const size_t FIELD_REAR_BIT_OFFSET = PREV_REAR + BITS;
		typedef INITIALIZER prev_initializer;
		typedef DUMPER prev_dumper;
	};

	template<size_t PREV_HEAD, size_t PREV_REAR, bool FORWARD>
	struct field_head_offset;

	template<size_t PREV_HEAD, size_t PREV_REAR>
	struct field_head_offset<PREV_HEAD, PREV_REAR, true>
	{
	public:
		static const size_t FIELD_HEAD_BIT_OFFSET = PREV_REAR;
	};
	...
	{
	public:
		static const size_t FIELD_HEAD_BIT_OFFSET = PREV_HEAD;
	};

	typedef uint8_t pkt_field_assist_type;

	template<typename T>
	struct type_bits
	{
	public:
		static const size_t TYPE_BITS = sizeof(T) * static_cast<size_t>(8);
	};
...
#    endif
	...
	{
	public:
		TYPE operator()(const p2engine::safe_buffer &buf, std::ptrdiff_t offset_in_buf, 
			size_t OFFSET_IN_TYPE, size_t BITS)
		{
			BOOST_ASSERT(offset_in_buf+sizeof(TYPE)<=buf.length());
			char* p=p2engine::buffer_cast<char*>(buf)+offset_in_buf;
			return (p2engine::read_int_ntoh<TYPE>(p)
	...
	{
	public:
		void operator()(const p2engine::safe_buffer& buf, std::ptrdiff_t offset_in_buf, 
			size_t OFFSET_IN_TYPE, size_t BITS, TYPE val)
		{
			BOOST_ASSERT(offset_in_buf+sizeof(TYPE)<=buf.length());
			char* p=p2engine::buffer_cast<char*>(buf)+offset_in_buf;
			char *rp=p, *wp=p;
			TYPE new_field_value=p2engine::read_int_ntoh<TYPE>(rp);
			...
			write_int_hton<TYPE>(new_field_value, wp);
