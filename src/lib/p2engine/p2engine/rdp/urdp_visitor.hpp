...
namespace p2engine { namespace urdp{
	...
	P2ENGINE_PACKET_FORMAT_DEF_BEGIN(urdp_packet_basic_format, 0, {})
		P2ENGINE_PACKET_BIT_FIELD_DEF(uint32_t, control, 8)
		P2ENGINE_PACKET_BIT_FIELD_DEF(uint32_t, peer_id, 24)
		P2ENGINE_PACKET_BIT_FIELD_DEF(uint32_t, lostrate_recving, 8)
		P2ENGINE_PACKET_BIT_FIELD_DEF(uint32_t, id_for_lost_detect, 8)
		P2ENGINE_PACKET_BIT_FIELD_DEF(uint32_t, session_id, 16)
		P2ENGINE_PACKET_BIT_FIELD_DEF(uint32_t, bandwidth_recving, 16)//the unit: BANDWIDTH_PRECISIONΪ
		P2ENGINE_PACKET_BIT_FIELD_DEF (uint32_t, window, 16)
    ...
    P2ENGINE_DERIVED_PACKET_FORMAT_DEF_BEGIN (urdp_packet_reliable_format, urdp_packet_basic_format, {})
		...
		P2ENGINE_PACKET_FIELD_DEF(uint32_t, seqno)
		P2ENGINE_PACKET_FIELD_DEF (uint32_t, ackno)
    P2ENGINE_DERIVED_PACKET_FORMAT_DEF_END
...
struct make_refuse_vistor<urdp::urdp_packet_basic_format> 
{
	...
	safe_buffer operator()(const header_format& h)
	{
		...
		safe_buffer buf(header_format::format_size());
...
struct get_demain_name_vistor<urdp::urdp_packet_basic_format> 
{
	...
	{
		if (urdp::urdp_packet_reliable_format::format_size()<buf.length())
		{
			if (is_conn_request_vistor<header_format>()(h))
			{
				safe_buffer domainBuf=buf.buffer_ref(urdp::urdp_packet_reliable_format::format_size());
...
template <>
struct is_refuse_vistorr<urdp::urdp_packet_basic_format> 
{
	typedef urdp::urdp_packet_basic_format header_format;
	bool operator()(const header_format& h)const
	{
		return h.get_control()==urdp::CTRL_NETWORK_UNREACHABLE;
	}
};
