...
namespace p2engine{ namespace urdp{
	...
	public:
		...
		typedef urdp_flow flow_impl_type;
		typedef typename flow_impl_type::shared_layer_impl_type shared_layer_impl_type;
	...
	protected:
		...
		void set_flow(flow_sptr flow)
		{
			...
			BOOST_ASSERT(boost::dynamic_pointer_cast<flow_impl_type>(flow) == boost::static_pointer_cast<flow_impl_type>(flow));
			error_code ec;
			cached_remote_endpoint_=flow->remote_endpoint(ec);
			flow_=boost::static_pointer_cast<flow_impl_type>(flow);
		}
	...
	public:
			...
			)
		{
			...
			{
				flow_=flow_impl_type::create_for_active_connect(this->get_io_service(), 
					SHARED_OBJ_FROM_THIS, local_edp, ec);
			}
			return ec;
		}
		...
		virtual safe_buffer make_punch_packet(error_code& ec, const endpoint& externalEdp)
		{
			...
			if (!flow_)
			{
				flow_impl_sptr flow_s=flow_impl_type::create_for_active_connect(
					this->get_io_service(), SHARED_OBJ_FROM_THIS, cached_remote_endpoint_, ec);
	...
	private:
		...
		{
			flow_impl_sptr flowHolder = flow_;
			state_=CLOSED;
			flow_.reset();
			if (!is_canceled_op(op_stamp()))
			{
	...
	private:
		void __close(bool greaceful)
		{
			...
			state_=CLOSED;
			this->unregister_all_dispatch_handler();
		}

	private:
		flow_impl_sptr flow_;
		std::string domain_;
		int state_;
