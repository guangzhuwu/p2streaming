...
namespace p2engine {
	...
	namespace urdp{
		...
		public:
			...
			typedef basic_shared_udp_layer shared_layer_impl_type;
			...
			enum TcpState {
				INIT, LISTEN, SYN_SENT, SYN_RCVD, ESTABLISHED, 
				CLOSING, WATING_FIN_ACK, CLOSED
			};
			enum SendFlags { sfNone, sfDelayedAck, sfImmediateAck };
			enum { RECV_BUF_SIZE = 0xffff + 16 * 1024, SND_BUF_SIZE = RECV_BUF_SIZE * 3 / 2 };

		protected:
			urdp_flow(io_service& ios);
			virtual ~urdp_flow();

		public:
			static shared_ptr create_for_active_connect(
				io_service& ios, 
			...
			void async_connect(const endpoint& remoteEnp, const std::string&domainName, 
		...
		private:
			...
			static inline bool mod_less(IntType n1, IntType n2)
			{
				static wrappable_less<IntType> comp;
				return comp(n1, n2);
			}
			template<typename IntType>
			static inline bool mod_less_equal(IntType n1, IntType n2)
			{
				static wrappable_less_equal<IntType> comp;
				return comp(n1, n2);
			}
			template<typename IntType>
			static inline long mod_minus(IntType n1, IntType n2)
			{
				static wrappable_minus<IntType> comp;
				return (long)comp(n1, n2);
			}
			static inline time32_type tick_now()
			{
			...
			typedef std::deque<SSegment, allocator<SSegment> > SSegmentList;
			typedef std::set<RSegment, std::less<RSegment>, allocator<RSegment> > RSegmentList;
			typedef std::deque<safe_buffer, allocator<safe_buffer> > RUnraliablePacketList;
			typedef std::set < SUnraliableSegment, std::less<SUnraliableSegment>, 
				allocator<SUnraliableSegment> > SUnraliableSegmentList;
			...
			bool __process(const safe_buffer& buf, const endpoint& from);
		...
		protected:
			...
			// Timestamp tracking
			uint16_t m_t_recent, m_t_recent_now;
		...
		protected:
			...
			shared_layer_impl_sptr m_shared_layer;
			uint32_t m_flow_id;
			...
			typedef std::map<seqno_mark_type, time32_type, std::less<seqno_mark_type>, 
				allocator<std::pair<const seqno_mark_type, time32_type> > > seqno_mark_map;
