...
namespace p2engine{
	namespace trdp{
		...
		public:
			...
			typedef basic_shared_tcp_layer shared_layer_impl_type;
			typedef boost::shared_ptr<basic_shared_tcp_layer> shared_layer_impl_sptr;
		...
		protected:
			...
			struct send_emlment:object_allocator
			{
				safe_buffer buf;
				msec_type   outTime;
				uint16_t    msgType;
		...
		public:
			static shared_layer_sptr create_shared_layer_for_acceptor(
				io_service& ios, 
				endpoint& local_edp, 
			...
			static shared_ptr create_for_active_connect(connection_sptr conn, 
				io_service& ios, const endpoint& local_edp, error_code& ec, 
				bool realTimeUtility = true
				)
			{
			...
			void __waiting_domain_coro(const error_code& ec, std::size_t len, op_stamp_t stamp, 
		...
		public:
			...
			virtual void set_flow_id(uint32_t id)
			{
				flow_id_ = id;
			}
			...
			virtual void on_received(const safe_buffer& data, const endpoint& from)
			{
				BOOST_ASSERT(0);
			}
			...
			error_code open(const endpoint& local_edp, error_code& ec)
			{
			...
			{
				return open(endpoint(), ec);
			}

			void connect(const std::string& remote_host, int port, 
				const std::string& domainName);

			void connect(const endpoint& remote_edp, const std::string& domainName);
			...
			void on_received_punch_request(safe_buffer& buf, const endpoint& remote_edp)
			{//not support yet
				(void)(buf);
				(void)(remote_edp);
			}
		...
		private:
			void do_keep_receiving(error_code ec, std::size_t len, op_stamp_t stamp, 
				coroutine coro = coroutine());

			void __async_resolve_connect_coro(error_code err, resolver_iterator itr, 
				op_stamp_t stamp, coroutine coro = coroutine(), resolver_query* qy = NULL);

			void __domain_request_coro(error_code ec, std::size_t len, op_stamp_t stamp, 
				coroutine coro = coroutine());

			bool __process_data(op_stamp_t stamp);
			...
			void __handle_sent_packet(const error_code& ec, std::size_t bytes_trans, 
		...
		private:
			boost::weak_ptr<shared_layer_impl_type> shared_layer_;
			...
			std::deque<send_emlment, allocator<send_emlment> > send_bufs_;
			int qued_sending_size_;
			bool sending_;

		protected:
			typedef wrappable_integer<int8_t> seqno_mark_type;
			typedef std::map<seqno_mark_type, msec_type> seqno_mark_map;
			mutable seqno_mark_map recvd_seqno_mark_for_lost_rate_, send_seqno_mark_for_lost_rate_;
