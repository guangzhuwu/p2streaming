...
namespace p2engine{
	...
	public:
		...
		{
			this->get_service().open(this->implementation, path, mode, ec);
		}
		...
		{
			this->get_service().open(this->implementation, path, read_write, ec);
		}
		...
		{
			this->get_service().async_open(this->implementation, path, mode, handler);
		}

		void seek(int64_t offset, int original, boost::system::error_code& ec)
		{
			this->get_service().seek(this->implementation, offset, original, ec);
		}
		...
		{
			this->get_service().async_write_some_at(this->implementation, 
		...
		{
			boost::system::error_code ec;
			size_t s = this->get_service().read_some_at(
				this->implementation, offset, buffers, ec);
		...
		{
			this->get_service().async_read_some_at(this->implementation, 
