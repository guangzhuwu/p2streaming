...
namespace p2engine { namespace http {
	...
	template<typename BasicConnection>
	class basic_http_connection
		: public BasicConnection
	{
		typedef basic_http_connection<BasicConnection> this_type;
		SHARED_ACCESS_DECLARE;
		BOOST_STATIC_ASSERT((boost::is_same<BasicConnection, http_connection_base>::value)
			||(boost::is_base_and_derived<http_connection_base, BasicConnection>::value));
		typedef detail::basic_http_connection_impl impl_type;

	protected:
		basic_http_connection(io_service& ios, bool enable_ssl, bool isPassive)
			:BasicConnection(ios, enable_ssl, isPassive)
		{}
	...
	public:
		typedef BasicConnection connection_base_type; 
		typedef typename BasicConnection::lowest_layer_type lowest_layer_type; 
	...
	public:
		...
		virtual size_t overstocked_send_size()const
		{
			BOOST_ASSERT(impl_);
			if (!impl_)
				return 0;
