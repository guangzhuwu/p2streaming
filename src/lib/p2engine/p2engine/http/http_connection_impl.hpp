...
namespace p2engine { namespace http { namespace  detail{
	...
	public:
		...
		size_t overstocked_send_size()const
		{
	...
	protected:
		...
		error_code __open(const endpoint& local_edp, error_code& ec);
		...
		void __async_send_handler(error_code ec, size_t len, op_stamp_t);

		void __async_recv_handler(error_code ec, size_t len, op_stamp_t);
