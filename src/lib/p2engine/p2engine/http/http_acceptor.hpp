...
namespace p2engine { namespace http {
	...
	template<typename basic_trdp_connection, typename ConnectionBaseType>
	class basic_http_acceptor
		:public http_acceptor_base<ConnectionBaseType>
		, public operation_mark
	{
		typedef basic_http_acceptor<basic_trdp_connection, ConnectionBaseType> this_type;
	...
	protected:
			...
			boost::shared_ptr<connection_type> conn, op_stamp_t stamp)
		{
			...
			basic_http_acceptor_dispatcher<connection_base_type>::dispatch_accepted(conn, ec);
