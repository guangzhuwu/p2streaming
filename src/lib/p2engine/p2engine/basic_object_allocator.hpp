...
namespace p2engine {
	...
	{
		typedef basic_object_allocator<UserAllocator> this_type;
		typedef basic_memory_pool<UserAllocator> memory_pool_type;

		static  void* operator new(size_t bytes)
		{
			void* rst=memory_pool_type::malloc(bytes);
			if (!rst)throw std::bad_alloc();
			return rst;
		}
		static void operator delete (void *p)
		{
			memory_pool_type::free(p);
		}

		static void *operator new(size_t bytes, const std::nothrow_t&)throw()
		{
			return memory_pool_type::malloc(bytes);
		}
		...
		static void *operator new(size_t bytes, void *ptr) throw()
		{
