...
#define MAX_PARAM 10
#ifndef BOOST_PP_ENUM_SHIFTED_PARAMS
...
#define P2ENGINE_ARGS_TEMPLATE_DECL(arity) \
	BOOST_PP_ENUM_SHIFTED_PARAMS(BOOST_PP_INC(arity), typename T)
// typename R, typename T1, typename T2, ..., typename TN
#define P2ENGINE_SIGNATURE_TEMPLATE_DECL(arity) \
	typename R BOOST_PP_COMMA_IF(arity) \
	BOOST_PP_ENUM_SHIFTED_PARAMS(BOOST_PP_INC(arity), typename T)
// T1, T2, ..., TN
#define P2ENGINE_ARGS_TEMPLATE_INSTANTIATION(arity) \
	BOOST_PP_ENUM_SHIFTED_PARAMS(BOOST_PP_INC(arity), T)
// R (T1, T2, ..., TN)
#define P2ENGINE_SIGNATURE_FUNCTION_TYPE(arity) \
	R(P2ENGINE_ARGS_TEMPLATE_INSTANTIATION(arity))
// argn
#define P2ENGINE_SIGNATURE_ARG_NAME(z, n, data) BOOST_PP_CAT(arg, BOOST_PP_INC(n))
// Tn argn
#define P2ENGINE_SIGNATURE_FULL_ARG(z, n, data) \
	BOOST_PP_CAT(T, BOOST_PP_INC(n)) P2ENGINE_SIGNATURE_ARG_NAME(~, n, ~)
// T1 arg1, T2 arg2, ..., Tn argn
#define P2ENGINE_SIGNATURE_FULL_ARGS(arity) \
	BOOST_PP_ENUM(arity, P2ENGINE_SIGNATURE_FULL_ARG, ~)
// arg1, arg2, ..., argn
#define P2ENGINE_SIGNATURE_ARG_NAMES(arity) BOOST_PP_ENUM(arity, P2ENGINE_SIGNATURE_ARG_NAME, ~)
...
#define BIND(z, paramN, _) \
	template< typename FuncPoint BOOST_PP_COMMA_IF(paramN) P2ENGINE_ARGS_TEMPLATE_DECL(paramN)>\
	connection bind(FuncPoint fp BOOST_PP_COMMA_IF(paramN) P2ENGINE_SIGNATURE_FULL_ARGS(paramN))\
				{\
				...
				connImpl->func() = boost::bind(fp BOOST_PP_COMMA_IF(paramN) P2ENGINE_SIGNATURE_ARG_NAMES(paramN)); \
...
#define SIGNAL_IMPL_R_RETURN(z, paramN, _) \
	template< P2ENGINE_SIGNATURE_TEMPLATE_DECL(paramN) >\
			class signal_impl< P2ENGINE_SIGNATURE_FUNCTION_TYPE(paramN) >\
			:public signal_base< P2ENGINE_SIGNATURE_FUNCTION_TYPE(paramN) >\
			{\
			typedef slot<P2ENGINE_SIGNATURE_FUNCTION_TYPE(paramN)> slot_type; \
			\
			\
			R _emit(P2ENGINE_SIGNATURE_FULL_ARGS(paramN) BOOST_PP_COMMA_IF(paramN) bool front = false)\
			{\
			...
			v = ((slot_type*)((*itr).get()))->func()(P2ENGINE_SIGNATURE_ARG_NAMES(paramN)); \
			if (front) break; \
			}\
			}\
			...
			\
			public:\
			R operator()(P2ENGINE_SIGNATURE_FULL_ARGS(paramN))\
			{\
			return _emit(P2ENGINE_SIGNATURE_ARG_NAMES(paramN)); \
			}\
			R emit(P2ENGINE_SIGNATURE_FULL_ARGS(paramN))\
			{\
			return _emit(P2ENGINE_SIGNATURE_ARG_NAMES(paramN)); \
			}\
			R emit_front(P2ENGINE_SIGNATURE_FULL_ARGS(paramN))\
			{\
			return _emit(P2ENGINE_SIGNATURE_ARG_NAMES(paramN) BOOST_PP_COMMA_IF(paramN) true); \
			}\
			};


#define SIGNAL_IMPL_VOID_RETURN(z, paramN, _) \
	template< P2ENGINE_ARGS_TEMPLATE_DECL(paramN) >\
			class signal_impl< void(P2ENGINE_ARGS_TEMPLATE_INSTANTIATION(paramN)) >\
			:public signal_base< void(P2ENGINE_ARGS_TEMPLATE_INSTANTIATION(paramN)) >\
			{\
			typedef slot<void(P2ENGINE_ARGS_TEMPLATE_INSTANTIATION(paramN))> slot_type; \
			\
			\
			void _emit(P2ENGINE_SIGNATURE_FULL_ARGS(paramN) BOOST_PP_COMMA_IF(paramN) bool front = false)\
			{\
			...
			{\
			BOOST_ASSERT(dynamic_cast<slot_type*>((*itr).get())); \
			((slot_type*)((*itr).get()))->func()(P2ENGINE_SIGNATURE_ARG_NAMES(paramN)); \
			if (front) break; \
			}\
			}\
			protector->dec_emiting(); \
			...
			\
			public:\
			void operator()(P2ENGINE_SIGNATURE_FULL_ARGS(paramN))\
			{\
			_emit(P2ENGINE_SIGNATURE_ARG_NAMES(paramN)); \
			}\
			void emit(P2ENGINE_SIGNATURE_FULL_ARGS(paramN))\
			{\
			_emit(P2ENGINE_SIGNATURE_ARG_NAMES(paramN)); \
			}\
			void emit_front(P2ENGINE_SIGNATURE_FULL_ARGS(paramN))\
			{\
			_emit(P2ENGINE_SIGNATURE_ARG_NAMES(paramN) BOOST_PP_COMMA_IF(paramN) true); \
			}\
			};

			BOOST_PP_REPEAT(BOOST_PP_INC(MAX_PARAM), SIGNAL_IMPL_R_RETURN, _)
				BOOST_PP_REPEAT(BOOST_PP_INC(MAX_PARAM), SIGNAL_IMPL_VOID_RETURN, _)
#undef SIGNAL_IMPL_R_RETURN
#undef SIGNAL_IMPL_VOID_RETURN
...
#define BIND(z, paramN, _) \
	template< typename FuncPoint BOOST_PP_COMMA_IF(paramN) P2ENGINE_ARGS_TEMPLATE_DECL(paramN)>\
	signal(FuncPoint fp BOOST_PP_COMMA_IF(paramN) P2ENGINE_SIGNATURE_FULL_ARGS(paramN))\
				{\
				this->bind(fp BOOST_PP_COMMA_IF(paramN) P2ENGINE_SIGNATURE_ARG_NAMES(paramN)); \
				}
