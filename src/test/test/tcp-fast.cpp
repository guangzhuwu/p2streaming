...
{
	fast_opts_ = 0;
	fast_opts_ |= FAST_ALLOW_CC;
	fast_opts_ |= FAST_ALLOW_MI;
	cwnd_update_time_ = fasttime();
	cwnd_increments = 0;
	firstrecv_ = -1.0;
	slowstart_ = 2;
	acks_per_rtt_ = 0;
	...
	if ( fast_opts_ & FAST_ALLOW_CC )
	{
			...
			cwnd_array_q >= beta_ * avgRTT_ )
		{
				...
				high_accuracy_cwnd_ == CWND_USE_INT )	//we use intergal to store cwnd.
			{
				...
				{
					cwnd_remainder_ += (target_cwnd - (int)target_cwnd);
					if (cwnd_remainder_ > 1)
					{
						target_cwnd++;
						cwnd_remainder_--;
					}
	...
	FastTcpAgent::fast_cc(double rtt, double old_cwnd)
{
	...
	if ( fast_calc_cwnd_end_ + rtt < currentTime_ ) {
		fast_calc_cwnd_end_ = currentTime_;
		acks_last_rtt_ = acks_per_rtt_;
		acks_per_rtt_ = 0;
		if ( on_first_rtt_ == true )	
			on_first_rtt_ = false;
		else	
			on_first_rtt_ = true;
	}
	...
	if ( avgRTT_ - baseRTT_ < mi_threshold_ && (fast_opts_ & FAST_ALLOW_MI) ) {
		cwnd_++;
		cwnd_update_time_ = currentTime_;
		return;
	}

	// If queueing delay is large, we use fast algorithm
	if ( currentTime_ >= cwnd_update_time_ + fast_update_cwnd_interval_ 
	...
	FastTcpAgent::fast_pace(TracedDouble *cwndp, int incre_4_cwnd)
{
	...
	double acks_per_period = (double)acks_last_rtt_ * fast_update_cwnd_interval_ / avgRTT_;

	if ( incre_4_cwnd >= 1 )
	{
		cwnd_increments += incre_4_cwnd;
		/* bc_spacing: target number of ACKs between increments */
		bc_spacing_ = (short unsigned int)(acks_per_period/cwnd_increments);
