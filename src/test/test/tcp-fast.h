...
protected:
	...
	double cwnd_remainder_;	//this variable is used to fix the calculation error using old_cwnd.
	double 		cwnd_update_time_;

	double		fast_update_cwnd_interval_;	
	double	 	cwnd_increments;		
	unsigned int 	acks_per_rtt_;
	unsigned int 	acks_last_rtt_;		
	unsigned short 	bc_ack_,bc_spacing_;	
	...
	double currentTime_;		//the time when a recv event happens
